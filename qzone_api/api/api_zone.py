...
class ApiZone(ApiBase):
    ...
    async def _get_zone(self, target_qq: int, g_tk: int, cookies: str,page:int=1,count:int=10,begintime:int=0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self._make_get_request(self.user_url, params, cookies=cookies)
            if response:
                # 创建保存目录
                save_dir = os.path.join(os.getcwd(), "data", "raw_messages")
                os.makedirs(save_dir, exist_ok=True)
                
                # 生成文件名 (使用时间戳和QQ号)
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"messages_{target_qq}_{timestamp}.json"
